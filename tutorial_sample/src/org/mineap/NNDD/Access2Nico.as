package org.mineap.NNDD
{
	...
	import org.mineap.a2n4as.NicoRankingUrl;
...
//		private var ngList:ArrayCollection;
		...
		public static const SEARCH_COMMENT_NEW:int = 4;
		public static const SEARCH_COMMENT_OLD:int = 5;
		public static const SEARCH_COMMENT_MANY:int = 6;
		public static const SEARCH_COMMENT_FEW:int = 7;
		...
		public static const NICO_SEARCH_SORT_VALUE:Array = new Array(
			"?sort=f","?sort=f&order=a","?sort=v","?sort=v&order=a","?sort=n","?sort=n&order=a","?sort=r","?sort=r&order=a","?sort=m","?sort=m&order=a","?sort=l","?sort=l&order=a"
		);
...
//			this.nnddvideodownload.requestDownload(mailAddress, password, videoId, null, new File(path));
		 ...
		 */
		public function request_rankingRenew(period:int, target:int, category:String, rankingListProvider:ArrayCollection, libraryManager:LibraryManager, pageIndex:int, tagsArray:ArrayCollection = null, label_status:Label = null):void
		{
			...
			listLoader = watchRanking(period, target, category);
		}
		...
		public function request_search(topPageUrl:String, loginUrl:String, 
			...
			searchWord:String, rankingListProvider:ArrayCollection, libraryManager:LibraryManager, sortIndex:int, pageIndex:int, tagArray:Array = null, label_statusInfo:Label = null):void
		{
			this.rankingListProvider = rankingListProvider;
			this.tagArray = tagArray;
...
//			trace(nicoSearchURL);
		 ...
		 * 外部向けAPIです。
		 * 
		 * @return ランキングに対応するビデオのURLとサムネイルのURLのリスト。
		 * 
		 */
...
//			trace("function:onLoginSuccess");
			...
			if(isVideoGetting){
				/* 先にコメントを取りにいく。コメントを取り終わったら２週目でビデオを取得しにいく。 */
...
//			trace(loader.data);
			var pattern1:RegExp = NicoPattern.searchVideoUrlAndTitlePattern;
			var pattern3:RegExp = NicoPattern.searchPageLinkPattern;
			var pattern4:RegExp = NicoPattern.searchNowPagePattern;
			
			var url_titleList:Array = loader.data.match(pattern1);
...
//			trace("\t"+pageLink);
			while(pageLink != null){
				...
				if(index != -1){
					...
					//HTML特殊文字を変換(&amp;→&)
					suffix = PathMaker.getSpecialCharacterNotIncludedVideoName(suffix);
					url = url + suffix;
			...
			for(var i:int = 0; max>0 && i<url_titleList.length; max--, i++)
			{
				var errorString:String;
				try{
					errorString = "ビデオURLの解析";
					var url:String = url_titleList[i].substring(url_titleList[i].indexOf("href=\"")+6,url_titleList[i].lastIndexOf("\" class"));
					errorString = "タイトルの解析";
					var title:String = url_titleList[i].substring(url_titleList[i].lastIndexOf("\">")+2,url_titleList[i].indexOf("</span>"));
					errorString = "サムネイルURLの解析";
					var thumbImage:String = PathMaker.getThumbImgUrl(PathMaker.getVideoID(url));
					urlList[i] = new Array("http://www.nicovideo.jp/" + url, title, thumbImage);
					...
					if(video != null){
						...
						if(video.isEconomy){
							videoCondition = "ビデオ(低画質)保存済\n右クリックから再生できます。"
						}else{
							videoCondition = "ビデオ保存済\n右クリックから再生できます。";
						}
					}
					
					var videoTitle:String = getSpecialCharacterNotIncludedVideoName(urlList[i][1]);
					
					rankingListProvider.addItem({
						dataGridColumn_ranking: index+((this.pageIndex-1)*30),
						dataGridColumn_preview: urlList[i][2],
					...
					this.getThumbInfo(url.substr(url.lastIndexOf("/")+1), i, false, videoTitle);
		 ...
		 * 
		 **/
		private function watchRanking(period:int, target:int, category:String):URLLoader
		{
...
//			statusLabel.text = "ランキングを更新しました";
			...
			if(nicoRankingURL.indexOf("ranking") == -1){
		...
		private function getRanking(loader:URLLoader, max:int=100):Array
		{
			...
			for each(var item:RankingItem in rAnalyzer.rankingItems){
				...
				try{
					...
					var videoTitle:String = item.title.substr(item.title.indexOf("位：") + 2);
					...
					this.getThumbInfo(videoId, index-1, false, videoTitle);
...
//			}
				...
				if(rankingListProvider != null && rankingIndex != -1 && rankingListProvider.length > rankingIndex){
					if(rankingVideoName != null && rankingVideoName == rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoName){
						this.rankingListProvider.setItemAt({
							...
							dataGridColumn_Info: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_Info,
							dataGridColumn_videoInfo: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoInfo,
							dataGridColumn_condition: "ビデオへアクセス中\n",
							dataGridColumn_videoPath: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoPath,
							dataGridColumn_date: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_date
						},rankingIndex);
					}
		 ...
		 * @param videoID 英数字２文字＋数字 で表されるビデオのID
...
//			statusLabel.text = "アドレスの取得に成功";
				...
				if(rankingListProvider != null){
					if(rankingVideoName != null && rankingIndex != -1 && rankingVideoName == rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoName){
						this.rankingListProvider.setItemAt({
							...
							dataGridColumn_Info: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_Info,
							dataGridColumn_videoInfo: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoInfo,
							dataGridColumn_condition: "ビデオ保存済\n右クリックから再生できます。",
				...
				if(rankingListProvider != null){
					if(rankingVideoName != null && rankingIndex != -1 && rankingVideoName == rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoName){
						this.rankingListProvider.setItemAt({
							...
							dataGridColumn_Info: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_Info,
							dataGridColumn_videoInfo: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoInfo,
							dataGridColumn_condition: "ビデオ保存失敗",
							dataGridColumn_videoPath: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoPath,
							dataGridColumn_date: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_date
						},rankingIndex);
					}
		...
		private function saveVideo(videoLoader:URLLoader):Boolean
		{
			...
			if(videoLoader.dataFormat != URLLoaderDataFormat.BINARY){
				...
				logManager.addLog("ビデオが正常にダウンロードできていませんでした。ダウンロードしたデータがバイナリではありません。（フォーマット:" + videoLoader.dataFormat + ",ファイル名:" + this.videoTitle + this.videoType + "）");
			...
			try{
				...
				if(!isNicowariGetting){
					...
					//タグ情報を読み込んでライブラリに反映
					var video:NNDDVideo = libraryManager.loadInfo(decodeURIComponent(file.url));
...
//				trace(path + videoTitle + videoType);
				...
				if(!isOwner){
					...
					fileIO.saveComment(commentLoader.data, fileName + videoType, filePath, false);
					fileIO.closeFileStream();
			...
			loader.addEventListener(Event.COMPLETE, function(event:Event):void{
				...
				try{
					...
					if(isSave){
						...
						fileIO.saveComment(loader.data, fileName + "[IchibaInfo].html", filePath, false);
						fileIO.closeFileStream();
		 ...
		 * @param videoName ランキング取得のときはデータグリッドのムービー名を入れる。
		 * 
		 */
		private function getThumbInfo(videoID:String, index:int, isSave:Boolean, videoName:String = null):void{
			//http://www.nicovideo.jp/api/getthumbinfo/動画ID
			var loader:URLLoader;
			loader = new URLLoader();
			loader.addEventListener(IOErrorEvent.IO_ERROR, function(event:Event):void{
				if(videoName == null){
					logManager.addLog("エラー:サムネイル情報の取得に失敗。\n対象のWebサービスが現在利用可能かどうか確認してください。");
					tagArray.push("タグ情報の取得に失敗");
...
//				trace(thumbInfoXML);
				...
				if(tagArray != null){
					for each(var tag:String in analyzer.tagArray){
						tagArray.push(PathMaker.getSpecialCharacterNotIncludedVideoName(tag));
					}
				}
				
				if(index != -1){
					if(videoName != null && rankingListProvider.length > index && videoName == rankingListProvider[index].dataGridColumn_videoName){
						rankingListProvider.setItemAt({
							dataGridColumn_ranking: rankingListProvider.getItemAt(index).dataGridColumn_ranking,
							dataGridColumn_preview: rankingListProvider.getItemAt(index).dataGridColumn_preview,
							dataGridColumn_videoName: rankingListProvider.getItemAt(index).dataGridColumn_videoName + videoNameFooter,
							dataGridColumn_Info: rankingListProvider.getItemAt(index).dataGridColumn_Info,
				...
				if(isSave){
					...
					try{
						...
						//ThumbInfo.xmlを保存
						fileIO.saveComment(loader.data, fileName + "[ThumbInfo].xml", filePath, false);
						fileIO.addFileStreamEventListener(IOErrorEvent.IO_ERROR, function(event:IOErrorEvent):void{
		...
		private function getThumbImg(thumbImgUrl:String, videoID:String, fileName:String, path:String):void{
			...
			imgLoader.addEventListener(Event.COMPLETE, function(event:Event):void{
				try{
					...
					/* すでに保存済のビデオならライブラリのNNDDVideoにthumbImgのURLを追加 */
					var video:NNDDVideo = libraryManager.isExist(videoID);
					if(video != null){
					...
					if(rankingVideoName != null && rankingVideoName.indexOf(rankingListProvider[rankingIndex].dataGridColumn_videoName) != -1){
						if(evt.currentTarget == this.commentLoader){
							...
							this.rankingListProvider.setItemAt({
								...
								dataGridColumn_Info: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_Info,
								dataGridColumn_videoInfo: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoInfo,
								dataGridColumn_condition: "ビデオをDL中\n" + new int((evt.bytesLoaded/evt.bytesTotal)*100) + "%\n" + 
										formatter.format(loadedValue)+"MB/"+formatter.format(totalValue)+"MB",
								dataGridColumn_videoPath: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_videoPath,
								dataGridColumn_date: rankingListProvider.getItemAt(rankingIndex).dataGridColumn_date
							},rankingIndex);
...
//				Alert.show("エラー：予期せぬエラーが発生しました。\n対象のWebサービスが現在利用可能かどうか確認してください。\n" + evt.target + ":" + evt);
		...
		/**
		 * ビデオのダウンロードをキャンセルし、DataGridおよびステータスラベルの更新を行います。
		 */
		public function videoDownloadCancel():void{
			...
			allClose(true);
			logManager.addLog("ビデオのダウンロードがキャンセルされました:" + videoTitle + videoType);
		}
