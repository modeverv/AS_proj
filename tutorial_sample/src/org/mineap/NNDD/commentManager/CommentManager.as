package org.mineap.NNDD.commentManager
{
	...
	public class CommentManager
	{
		 ...
		 * 
		 */
		public function initComment(comments:Comments, displayObject:DisplayObjectContainer, windowType:int):Boolean{
		...
		/**
		 * 指定されたvposとmailを使ってムービー再生用ビューにコメントをセットします。
					...
					if(comment.text != ""){
						...
						if(isShow){
							switch(commandPosition){
								case Command.UE:
									result = this.addUeComment(comment.vpos, comment.text, command.getSize(comment.mail), command.getColorByCommand(comment.mail), comment.no, comment.mail);
									break;
								case Command.SHITA:
									result = this.addShitaComment(comment.vpos, comment.text, command.getSize(comment.mail), command.getColorByCommand(comment.mail), comment.no, comment.mail);
									break;
								case Command.NAKA:
								default:
									result = this.addNomalComment(comment.vpos, comment.text, command.getSize(comment.mail), command.getColorByCommand(comment.mail), comment.no, comment.mail);
									break;
							}
						}
					}
		 ...
		 * 
		 */
		private function addNomalComment(vpos:int, comment:String, size:int, color:int, no:Number, mail:String):Boolean
		{
			for(var j:int = 0; j<commentNomalTextArray.length; j++){
				for(var i:int = 0; i<commentNomalTextArray[j].length; i++){
		 ...
		 * 
		 */
		private function addUeComment(vpos:int, comment:String, size:int, color:int, no:Number, mail:String):Boolean
		{
...
//						(commentUeTextArray[i][1] as NNDDText).y = (int)(newY + (commentUeTextArray[i][1] as NNDDText).textHeight);
		 ...
		 * 
		 */
		private function addShitaComment(vpos:int, comment:String, size:int, color:int, no:Number, mail:String):Boolean
		{
...
//				commentText.setStyle("fontAntiAliasType", flash.text.AntiAliasType.ADVANCED);
		...
		public function setCommentBold(isCommentBold:Boolean):void{
			...
			try{
				...
				trace("まだ準備が終わってない:"+error.getStackTrace());
			}
		}
