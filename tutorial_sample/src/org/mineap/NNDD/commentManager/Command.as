package org.mineap.NNDD.commentManager
{
	...
	public class Command
	{
		...
		public static const POSITION_PATTERNS:Array = new Array(
			new RegExp(POSITION_ARRAY[0], "ig"), new RegExp(POSITION_ARRAY[1], "ig"), new RegExp(POSITION_ARRAY[2], "ig")
		);
		...
		public static const SIZE_PATTERNS:Array = new Array(
			new RegExp(SIZE_ARRAY[0], "ig"), new RegExp(SIZE_ARRAY[1], "ig"), new RegExp(SIZE_ARRAY[2], "ig")
		);
		...
		public static const COLLOR_COMMAND_PATTERNS:Array = new Array(
			new RegExp(COLLOR_COMMAND_ARRAY[0], "ig"), new RegExp(COLLOR_COMMAND_ARRAY[1], "ig"), new RegExp(COLLOR_COMMAND_ARRAY[2], "ig"), 
			new RegExp(COLLOR_COMMAND_ARRAY[3], "ig"), new RegExp(COLLOR_COMMAND_ARRAY[4], "ig"), new RegExp(COLLOR_COMMAND_ARRAY[5], "ig"), 
			new RegExp(COLLOR_COMMAND_ARRAY[6], "ig"), new RegExp(COLLOR_COMMAND_ARRAY[7], "ig"), new RegExp(COLLOR_COMMAND_ARRAY[8], "ig")
		);
		...
		public static const COLLOR_PREMIUM_COMMAND_PATTERNS:Array = new Array(
			new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[0], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[1], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[2], "ig"), 
			new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[3], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[4], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[5], "ig"), 
			new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[6], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND_ARRAY[7], "ig")
		);
		...
		public static const COLLOR_PREMIUM_COMMAND2_PATTERNS:Array = new Array(
			new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[0], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[1], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[2], "ig"), 
			new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[3], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[4], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[5], "ig"), 
			new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[6], "ig"), new RegExp(COLLOR_PREMIUM_COMMAND2_ARRAY[7], "ig")
		);
			...
			if(command.length > 0){
				for(var index:int = 0; index < Command.POSITION_ARRAY.length; index++){
					if(command.match(Command.POSITION_PATTERNS[index]).length >= 1){
						//blueとtureをueと誤認しないかどうか
						if((command.indexOf(Command.COLLOR_COMMAND_ARRAY[Command.BLUE]) == -1) &&
								(command.indexOf(Command.COLLOR_PREMIUM_COMMAND_ARRAY[Command.RED2]) == -1)){
							return index;
						}
					}
				}
		 ...
		 * コマンドであり、かつ@CM命令であれば、指定されているニコ割ビデオのIDと、
		 * NICOWARI_PLAY or NICOWARI_STOPのいずれかを返します。
		 * 
		 * @param command
		 * @return Array("ニコ割ビデオID", NICOWARI_PLAY or NICOWARI_STOP, "再生開始時刻(hhmm形式)")
		 * 	ニコ割ビデオID:ニコ割のビデオID。存在しなければ空の文字列。
		 ...
		 * 
		 * @param command
		 * @return Array("ビデオID", "ジャンプメッセージ")<br>
		 * 	ビデオID:ジャンプ先のビデオID<br>
