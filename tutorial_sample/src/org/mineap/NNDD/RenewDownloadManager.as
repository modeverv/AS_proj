package org.mineap.NNDD
{
	...
	public class RenewDownloadManager extends EventDispatcher
	{
		 ...
		 * 
		 */
		public function renewForCommentOnly(user:String, password:String, videoId:String, videoName:String, saveDir:File, isReNameOldComment:Boolean):void{
			//失敗系ハンドラ登録
			...
			this._nnddDownloader.requestDownloadForCommentOnly(user, password, videoId, videoName, saveDir, false, isReNameOldComment);
		}
		 ...
		 * 
		 */
		public function renewForOtherVideo(user:String, password:String, videoId:String, videoName:String, saveDir:File, isReNameOldComment:Boolean):void{
			//失敗系ハンドラ登録
			this._nnddDownloader.addEventListener(NNDDDownloader.COMMENT_GET_FAIL, getFailListener, false, 0, true);
			this._nnddDownloader.addEventListener(NNDDDownloader.GETFLV_API_ACCESS_FAIL, getFailListener, false, 0, true);
			this._nnddDownloader.addEventListener(NNDDDownloader.ICHIBA_INFO_GET_FAIL, getFailListener, false, 0, true);
			...
			this._videoName = videoName;
			this._nnddDownloader.requestDownloadForOtherVideo(user, password, videoId, videoName, saveDir, false, isReNameOldComment);
		}
		...
		public function getSuccessListener(event:Event):void{
			...
			if(event.type == NNDDDownloader.LOGIN_SUCCESS){
				...
				status = "動画ページアクセス成功";
				dispatchEvent(new Event(NNDDDownloader.WATCH_SUCCESS));
