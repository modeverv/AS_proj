...
package deng.fzip
{
	...
	public class FZipFile
	{
		 ...
		 * @private
		 */		
		// M4 check - private static var HAS_INFLATE:Boolean = (describeType(ByteArray)..method.(@name == "inflate").length() == 1);
		/** Check that uncompress has at least one parameter **/	
		private static var HAS_INFLATE:Boolean = describeType(ByteArray)..method.(@name == "uncompress" )[0].children().length() == 1;
		...
		protected function parseContent(data:IDataInput):void {
			...
			if(_compressionMethod === COMPRESSION_DEFLATED && !_encrypted) {
				if(HAS_INFLATE) {
					...
					// M5 change:
					var uncompressFunc : Function = _content["uncompress"] as Function;
					data.readBytes(_content, 0, _sizeCompressed);
					uncompressFunc.apply( _content, [ "deflate" ] );
					_content.position = 0;
