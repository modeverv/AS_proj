...
// tt_configure tooltips individually. Individual commands override global
// tt_configuration. Order of commands is arbitrary.
...
tt_config. FadeIn			= 0 		// Fade-in duration in ms, e.g. 400; 0 for no animation
tt_config. FadeOut 		= 0
...
// OnLoadtt_config, OnCreateContentString, OnSubDivsCreated, OnShow, OnMoveBefore,
// OnMoveAfter, OnHideInit, OnHide, OnKill

var tt_aElt = new Array(10), // Container DIV, outer title & body DIVs, inner title & body TDs, closebutton SPAN, shadow DIVs, and IFRAME to cover windowed elements in IE
tt_aV = new Array(),	// Caches and enumerates tt_config data for currently active tooltip
...
function tt_Init()
{
	...
	// the user if they've forgotten to set the TagsToTip tt_config flag
...
// Creates command names by translating tt_config variable names to upper case
...
function tt_HideSrcTags()
{
	...
	if(!tt_HideSrcTagsRecurs(tt_body))
		tt_Err("To enable the capability to convert HTML elements to tooltips,"
				+ " you must set TagsToTip in the global tooltip tt_configuration"
				+ " to true.");
}
...
function tt_ReadCmds(a)
{
	...
	// First load the global tt_config values, to initialize also values
	// for which no command has been passed
	i = 0;
	for(var j in tt_config)
		tt_aV[i++] = tt_config[j];
	// Then replace each cached tt_config value for which a command has been
	...
	tt_aElt[0].innerHTML =
		...
		+ (tt_aV[TITLE].length ?
			...
			+ (tt_aV[CLOSEBTN] ?
				...
				+ ';" >' ///xonmouseover="tt_OnCloseBtnOver(1)" xonmouseout="tt_OnCloseBtnOver(0)" xonclick="tt_HideInit()"
				+ tt_aV[CLOSEBTNTEXT]
				+ '</span></td>')
				: '')
			+ '</tr></tbody></table></div>')
		...
		if(tt_iState & 0x4)
		{
			...
			if(!tt_op && !tt_ie)
			{
				if(tt_bWait)
					return;
				tt_bWait = true;
				tt_tWaitMov.Timer(function(){tt_bWait = false;}, 1, true);
			}
			if(tt_aV[FIX])
			{
				tt_iState &= ~0x4;
...
function tt_Fade(a, now, z, n)
{
	if(n)
	{
		...
		if((z > a) ? (now >= z) : (now <= z))
			...
			tt_tFade.Timer(function(){ tt_Fade(
							a ,  now , z , (n - 1)
							)},
							tt_aV[FADEINTERVAL],
							true);
