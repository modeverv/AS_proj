package 
{
	...
	public class Main extends Sprite 
	{
		...
		private var bounce:Number = -0.99;
		private var friction:Number = 0.995;
...
//			graphics.lineTo(particles[1].x, particles[1].y);			
		...
		private function spring(partA:WeightBall, partB:WeightBall):void
		{
			var dx:Number = partB.x - partA.x;
			var dy:Number = partB.y - partA.y;
			var dist:Number = Math.sqrt(dx * dx + dy * dy);
			if(dist < minDist)
			{
				graphics.lineStyle(1, 0xffffff * Math.random(),1 - dist/minDist);
