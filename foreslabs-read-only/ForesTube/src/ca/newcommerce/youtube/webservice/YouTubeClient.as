...
package ca.newcommerce.youtube.webservice
{
	...
	{	
		...
		public function login(username:String, password:String):Number
		{
			var request:URLRequest = new URLRequest("https://www.google.com/youtube/accounts/ClientLogin");
			var headers:Array = [];
			...
			variables.source = "youtube_discovery_test";
			
			request.data = variables;
			request.requestHeaders = headers;
			
			return runLoader(request, doLogin, { type:"login" });
		}
		
		public function rate(videoId:String, rating:Number):Number
		{
			var request:URLRequest = new URLRequest("http://gdata.youtube.com/feeds/api/videos/" + videoId + "/ratings");
			request.method = URLRequestMethod.POST;
			
			var headers:Array = [];
			headers.push(new URLRequestHeader("Content-Type", "application/atom+xml"));
			headers.push(new URLRequestHeader("Authorization", "GoogleLogin auth=" + _auth));
			headers.push(new URLRequestHeader("X-GData-Client", _clientId));
			headers.push(new URLRequestHeader("X-GData-Key", "key="+_developerKey));
		...
		//	headers.push(new URLRequestHeader("Content-Length", ""+content.length));
			
			request.requestHeaders = headers;
			request.data = content;
			
			return runLoader(request, doRating, { videoId:videoId, rating:rating } );
		}
		
		public function favorite(videoId:String):Number
		{
			var request:URLRequest = new URLRequest("http://gdata.youtube.com/feeds/api/users/default/favorites");
			request.method = URLRequestMethod.POST;
			...
			return runLoader(request, doFavorite, { videoId:videoId } );
		}
		
		public function unfavorite(videoId:String):Number
		{
			var request:URLRequest = new URLRequest("http://gdata.youtube.com/feeds/api/users/" + _username + "/favorites/" + videoId);
			...
			return runLoader(request, doUnfavorite, { videoId:videoId } );
		}
		
		protected function doUnfavorite(evt:Event):void
		{
			trace("doUnfavorite");			
		}
		...
		protected function doRating(evt:Event):void
		{
			...
			dispatchEvent(new YouTubeEvent(YouTubeEvent.RATING_SUCCESSFULL, wrapper));
		}
		...
		protected function doLogin(evt:Event):void
		{
			...
			_auth = authRegEx.exec(evt.target.data as String)[1];
			_username = userRegEx.exec(evt.target.data as String)[1];
			
			dispatchEvent(new YouTubeEvent(YouTubeEvent.LOGIN_SUCCESSFULL, wrapper));			
		}
