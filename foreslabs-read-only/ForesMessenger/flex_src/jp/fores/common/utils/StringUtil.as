package jp.fores.common.utils
{
	...
	public class StringUtil
	{
		 ...
		 * キロ
		 */
		public static const KILO :Number = 1024;
		
		/**
		 * メガ
		 */
		public static const MEGA :Number = KILO * KILO;
		
		/**
		 * ギガ
		 */
		public static const GIGA :Number = MEGA * KILO;
		
		/**
		 * テラ
		 */
		public static const TERA :Number = GIGA * KILO;
		
		/**
		 * ペタ
		 */
		public static const PETA :Number = TERA * KILO;
		
		/**
		 * エクサ
		 */
		public static const EXA :Number = PETA * KILO;
		
		/**
		 * ゼタ
		 */
		public static const ZETTA :Number = EXA * KILO;
		
		/**
		 * ヨタ
		 */
		public static const YOTTA :Number = ZETTA * KILO;
		
		/**
		 * 単位の数値の配列
		 */
		public static const UNIT_NUMBER_ARRAY :Array = [KILO, MEGA, GIGA, TERA, PETA, EXA, ZETTA, YOTTA];
		
		/**
		 * 単位の文字の配列
		 */
		public static const UNIT_WORD_ARRAY :Array = ["K", "M", "G", "T", "P", "E", "Z", "Y"];
		...
		public static function setComma(valueArg :Object) :String
		{
			...
			//この行に到達することはないはずだが、何か書かないとコンパイルエラーになるので、とりあえず「0」を返す
			return "0";
		}
		...
		/**
		 * 数値、または数値の入った文字列を、ファイルサイズ表示用の文字列に整形して返します。<br>
		 * 数値フォーマット用のインスタンスが指定されていない場合は、小数点第一位までの数値の後ろに単位の文字を結合した文字列を返す。<br>
		 * 変換できない不正な値が渡された場合は、「0」を返します。<br>
		 *
		 * @param valueArg 数値、数値の入った文字列
		 * @param formatter 数値フォーマット用のインスタンス
		 * @return 整形した文字列
		 */
		public static function formatFileSize(valueArg :Object, formatter :NumberFormatter = null) :String
		{
			//引数の値がnullの場合
			if(valueArg == null)
			{
			...
			try
			{
				...
				//引数の値を数値に変換
				var num :Number = Number(valueArg);
				...
				//単位の数値の配列の要素数に応じてループをまわす
				//(キロの分はすでにチェックしたので、インデックスは1から始める)
				for(var i :int = 1; i < UNIT_NUMBER_ARRAY.length; i++)
				{
					//引数の数値よりも現在の単位の数値の方が大きい場合
					if(num < UNIT_NUMBER_ARRAY[i])
					{
						//インデックスを1つ前に戻す
						i--;
				...
				//引数の数値を単位の数値で割る
				num = num / UNIT_NUMBER_ARRAY[i];
				
				//単位の文字を取得
				var unitWord :String = UNIT_WORD_ARRAY[i];
				
				
				//==========================================================
				//フォーマット
				
				//引数の数値フォーマット用のインスタンスが指定されていない場合
				if(formatter == null)
				{
					//数値フォーマット用のインスタンスを生成
				...
				//フォーマットした結果の文字列に単位の文字を結合して返す
				return formatter.format(num) + unitWord;
			}
			//例外処理
			//(数値に変換できない不正な値が渡された場合)
			catch(e :Error)
			{
				//「0」を返す
				return "0";
		...
		/**
		 * 数値、または数値の入った文字列を、ファイルサイズ表示用の文字列に整形して返します。(DataGridのlabelFunction用)<br>
		 * 数値フォーマット用のインスタンスが指定されていない場合は、小数点第一位までの数値の後ろに単位の文字を結合した文字列を返す。<br>
		 * 変換できない不正な値が渡された場合は、「0」を返します。<br>
		 *
		 * @param itemArg DataGridアイテムオブジェクト
		 ...
		 * @return 整形した文字列
		 */
		public static function formatFileSizeForDataGrid(itemArg :Object, columnArg :DataGridColumn) :String
		{
			//実際に処理を行うメソッドを呼び出す
			return formatFileSize(itemArg[columnArg.dataField]);
		}
